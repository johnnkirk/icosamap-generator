...
int main(argc,argv) int argc;char **argv; { int yasp,xasp;float ratio,len;
  ...
  for(vert=0;vert<12;++vert) {
    for(vcount=0;vcount<12;++vcount) {
      ...
      printf(" %10.7f",len=geogdist(&(parms.vertex[vert]),
                                    &(parms.vertex[vcount])));
      if((len>1.0)&&(len<2.0)) {
        segctr(&(parms.vertex[vert]),&(parms.vertex[vcount]),&geog);
        printf(" %7.4f %7.4f",geog.lat,geog.lon);
        printf(" -- %10.7f",geogdist(&geog,&(parms.vertex[vert])));
        printf(" %10.7f",geogdist(&geog,&(parms.vertex[vcount]))); }
      printf("\n"); }
    printf("\n");
    getch(); }
